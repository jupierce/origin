...
func (c *DockerCLI) GetImageID(imageTag string) (string, error) {
	...
	for _, img := range images {
		...
		if contains(img.RepoTags, imageTag) {
			imageID = img.ID
			e2e.Logf("get imageID is %s", imageID)
			break
		}
	}
	return imageID, nil
...
func (c *DockerCLI) RemoveImage(imageIndex string) (bool, error) {
	...
	_, err = c.CLI.ImageRemove(ctx, imageID, image.RemoveOptions{Force: true})
	if err != nil {
...
func (c *DockerCLI) GetImageList() ([]string, error) {
	...
	for _, img := range images {
		e2e.Logf("image: %s\n", strings.Join(img.RepoTags, ","))
		imageList = append(imageList, strings.Join(img.RepoTags, ","))
	}
...
func (c *DockerCLI) CheckImageExist(imageIndex string) (bool, error) {
	...
	for _, image := range imageList {
		if strings.Contains(image, imageIndex) {
			return true, nil
		}
	}
	return false, nil
...
// RemoveContainer is to remove container
func RemoveContainer(cli client.CommonAPIClient, id string) error {
	ctx := context.Background()
	err := cli.ContainerRemove(ctx, id, container.RemoveOptions{Force: true})
	return err
}

// StartContainer is to start a container
func StartContainer(cli client.CommonAPIClient, id string) error {
	ctx := context.Background()
	err := cli.ContainerStart(ctx, id, container.StartOptions{})
	return err
}

// ContainerStart is to start a container
func ContainerStart(ctx context.Context, cli client.CommonAPIClient, id string) error {
	// Convert our ContainerStartOptions to the SDK's container.StartOptions
