...
import (
	...
	"github.com/openshift/origin/test/extended/util/architecture"
	"github.com/tidwall/sjson"
	e2e "k8s.io/kubernetes/test/e2e/framework"
)

const (
	// NodeArchitectureLabel is the label key for node architecture
	NodeArchitectureLabel = "kubernetes.io/arch"
)

// Type aliases for OTP compatibility
type Architecture = architecture.Architecture

// Architecture constants for OTP compatibility
const (
...
// ArchitectureFromString is an alias for architecture.FromString
var ArchitectureFromString = architecture.FromString
...
func ListWorkerMachineSetNamesByArch(oc *exutil.CLI, arch architecture.Architecture) []string {
	...
	for _, workerMachineSetName := range workerMachineSetNames {
		...
		o.Expect(err).NotTo(o.HaveOccurred())
		if strings.Contains(machineSetAnnotation, NodeArchitectureLabel+"="+arch.String()) && !strings.Contains(machineSetLabels, `"machine.openshift.io/os-id":"Windows"`) {
